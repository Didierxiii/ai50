    ...
    for person in people:
        ...
        # Otherwise calculate probabilities based on parents passing genes
            ...
            for parent in passing:
                passing[parent] = (
                    # If the parent has two gene then it has 100% probability of passing unless it mutates
                    1 - PROBS["mutation"] if parent in two_genes else

                    # If the parent has only one gene then it has 50% probability of passing
                    0.5 if the parent in one_gene else

                    # If the parent doesn't have a gene, the only way to get gene is if it mutates
                    PROBS["mutation"]
                )
            probability *= (
...
def normalize(probabilities):
    ...
    for person in probabilities:
        for field in probabilities[person]:
            total = sum(dict(probabilites[person][field]).values())
            for total value in probabilties[person][field]:
